...
param(
    ...
    [string]$XCSIFInstallRoot = $PSScriptRoot,
    ...
    [bool]$SkipDeployStorefrontPackages = $true,
    ...
    [string]$CommerceEngineWdpFullPath = "$XCInstallRoot\Sitecore.Commerce.Engine.OnPrem.Solr.*custom.scwdp.zip",

    # The prefix that will be used on SOLR, Website and Database instances. The default value matches the Sitecore XP default.
    [string]$SiteNamePrefix = "ajax",
    # The name of the Sitecore site instance.
    [string]$SiteName = "$SiteNamePrefix.cm",
    # The name of the Sitecore CD site instance.
    [string]$SiteNameCD = "$SiteNamePrefix.cd",
    ...
    [string]$CommerceEngineConnectClientSecret = "XKjXTTjTws+IgtmGD0PojlJ2flE1JOocZW5mlOBuETY=",
    # The host header name for the Sitecore storefront site.
    [string]$SiteHostHeaderName = "$SiteName",

    # The path to the inetpub folder where Commerce is installed.
    [string]$CommerceInstallRoot = "$XCInstallRoot\..\..\www\",
    # The path of the Sitecore XP site.
    [string]$InstallDir = "$($CommerceInstallRoot)$SiteName",
    # The path of the Sitecore XP CD site.
    [string]$InstallDirCD = "$($CommerceInstallRoot)$SiteNameCD",
    # The path of the Sitecore XConnect site.
    [string]$XConnectInstallDir = "$($CommerceInstallRoot)$SiteNamePrefix.xconnect",
    ...
    [string]$SearchIndexPrefix = "$($SiteNamePrefix)_",
    # The URL of the Solr Server.
    [string]$SolrUrl = "https://localhost:8983/solr",
    # The folder that Solr has been installed to.
    [string]$SolrRoot = "$($Env:SYSTEMDRIVE)\$SiteNamePrefix-solr-8.4.0",
    # The name of the Solr Service.
    [string]$SolrService = "$SiteNamePrefix-solr-8.4.0",
    ...
    [string]$CommerceServicesDbName = "$($SiteNamePrefix)_SharedEnvironments",
    # The name of the global database for the Commerce Services.
    [string]$CommerceServicesGlobalDbName = "$($SiteNamePrefix)_Global",
    ...
    # The prefix appended to Commerce services folders names and sitenames.
    # The prefix allows you to host more than one Commerce installment on one server.
    [string]$CommerceServicesPrefix = "$SiteNamePrefix",
    ...
    [string]$CommerceServicesHostPostfix = "$SiteNamePrefix.ecom",

    # The name of the Sitecore XC Business Tools server.
    [string]$BizFxSiteName = "$SiteNamePrefix.bizfx",
    ...
    [array]$MinionEnvironments = @("$($SiteNamePrefix)Minions"),
    # whether to deploy sample data for each environment.
    [bool]$DeploySampleData = $false,
    ...
    [string]$UserName = "$SiteNamePrefix-EngineUser",
...
if ($SkipDeployStorefrontPackages -eq $false) {
    throw "SXA Storefront has been skipped in this installation script, so [SkipDeployStorefrontPackages] cannot be [false]."
}

if ($DeploySampleData -eq $true) {
    throw "SXA Storefront has been skipped in this installation script so [DeploySampleData] parameter cannot be set to [true]."
}
...
$deployCommerceParams = @{
    ...
    CommerceServicesPrefix                   = $CommerceServicesPrefix
